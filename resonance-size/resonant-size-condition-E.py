...
for i,d in enumerate(ds):
    ...
    Econd_CHIEF = torch.linalg.cond(E_CHIEF)
    ...
    Econds_CHIEF.append(Econd_CHIEF.item())
...
plt.subplot(2,1,1)
plt.scatter(Econds, ps, label = 'H')
plt.scatter(Econds_CHIEF, ps_CHIEF, label = 'H CHIEF')
plt.ylabel('Pressure (Pa)')
plt.xlabel('Cond')
plt.legend()

plt.subplot(2,1,2)
plt.scatter(ds, Econds, label = 'H')
plt.scatter(ds, Econds_CHIEF, label = 'H CHIEF')
